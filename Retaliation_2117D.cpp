...
void solve(int tt) {
    ...
    int n_dash = arr[0];
    fr(i,1,n) {
        if ((arr[i]/(i+1.0)) != (double) n_dash) {
            fromBegin = false;
            break;
        }
    }

    double n_dash_2 = arr[0]/((double)n);
    fr(i,1,n) {
        if ((arr[i]/(n-(i+1.0)-1.0)) != (double) n_dash_2) {
            fromEnd = false;
            break;
        }
    }
    ...
    double c1 = 1, c2 = n;
    double c1_dash = 2, c2_dash = n-1;

    double a1 = arr[0], a2 = arr[1];

    double n1 = (c2_dash * a1 - c2*a2)/(c2_dash*c1 - c2*c1_dash);
    double n2 = (c1*a2 - a1*c1_dash)/(c2_dash*c1 - c2*c1_dash);
