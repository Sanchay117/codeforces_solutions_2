//
// Created by bravefart69 on 10/1/25.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
	...
	if(n%2==0) {
		if((n&(n-1))==0) {
			...
			out(2*__bit_floor(n)-1);
			fr(i,1,n+1) {
				if(i!=n && i!=n-1 && i!=__bit_floor(n)-1) ans.pb(i);
			}
			ans.pb(n);
			ans.pb(n-1);
			ans.pb(__bit_floor(n)-1);
			for(auto x:ans) cout << x << " ";
			cout << endl;
		}
	}else {
		...
		// bug(l);
		fr(i,1,n+1) {
			if(i!=l && i!=l+(l==1?2:1) && i!=n-l && i!=n) ans.pb(i);
		}
		ans.pb(l);ans.pb(l+(l==1?2:1));
		ans.pb(n-l);ans.pb(n);
