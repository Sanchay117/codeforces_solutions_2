...
void solve(int tt) {
	...
	for( auto [k,v]:freq) {
		if(v>=2) {
			fr(i,0,v/2) {
				points_with_freq2.pb(k);
			}
		}
	}

	if(points_with_freq2.size()<4) {
		nahi;
		return;
	}
	...
	x1 = points_with_freq2[0],x2 = points_with_freq2.back(),y1 = points_with_freq2[1],y2 = points_with_freq2[points_with_freq2.size()-2];
	swap(x2,y2);
