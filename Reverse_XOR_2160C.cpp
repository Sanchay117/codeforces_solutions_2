...
void solve(int tt) {
    ...
    // 1. Convert n to a binary string (MSB first, like Python's binary string)
    string s = "";
    if (n_val == 0) s = "0";
    else {
    ...
    // 2. Iterate lengths from 1 to 63 (matching Python's range(1, 64))
    for (int i = 1; i < 64; ++i) {
        // If n is longer than i, it can't fit (Python: if len(n) <= i)
        if (L > i) continue;

        int pad = i - L; // How many leading zeros we need to imagine

        // --- CHECK 1: Palindrome ---
        // Python: B = w == w[::-1]
        bool is_palindrome = true;
        for (int k = 0; k < i / 2; ++k) {
            // Get char at left index k (simulating padding)
            char left = (k < pad) ? '0' : s[k - pad];

            // Get char at right index (i - 1 - k)
            int r_idx = i - 1 - k;
            char right = (r_idx < pad) ? '0' : s[r_idx - pad];
        ...
        // --- CHECK 2: Middle Bit (for odd lengths) ---
        // Python: if i%2==1: B = B and w[i//2]=='0'
        if (is_palindrome && (i % 2 == 1)) {
            int mid = i / 2;
            char mid_char = (mid < pad) ? '0' : s[mid - pad];
            if (mid_char != '0') {
                is_palindrome = false;
            }
        }
