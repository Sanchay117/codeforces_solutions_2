//
// Created by bravefart69 on 28/3/25.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
    ...
    if((map[x].F == map[y].F) || (map[x].F == map[y].S)) {
        ans[1] = map[x].F;
        ans[0] = map[x].S;
        if(map[x].F == map[y].F) ans[2] = map[y].S;
        else ans[2] = map[y].F;
    }else {
        ans[1] = map[x].S;
        ans[0] = map[x].F;
        if(map[x].S == map[y].F) ans[2] = map[y].S;
        else ans[2] = map[y].F;
    }
    ...
    if((map[x].F == map[y].F) || (map[x].F == map[y].S)) {
        ans[4] = map[x].F;
        ans[3] = map[x].S;
        if(map[x].F == map[y].F) ans[5] = map[y].S;
        else ans[5] = map[y].F;
    }else {
        ans[4] = map[x].S;
        ans[3] = map[x].F;
        if(map[x].S == map[y].F) ans[5] = map[y].S;
        else ans[5] = map[y].F;
    }
