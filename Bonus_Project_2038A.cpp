//
// Created by bravefart69 on 9/1/25.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
	...
	int brr[n];fr(i,0,n) cin >> brr[i];

	vector<tuple<int,int,int>> list; // ai,bi,ind

	fr(i,0,n) {
		tuple<int,int,int> tup = make_tuple(arr[i],brr[i],i);
		list.emplace_back(tup);
	}

	int ans = 0;

	sort(list.begin(),list.end(),[](tupiii a , tupiii b) {
	...
	for(tupiii x:list) {
		int greedy = get<0>(x)/get<1>(x);
		// bug(greedy , get<2>(x));
		if(ans+greedy>k) greedy = k - ans;
		ans+= greedy;
		anss.emplace_back(greedy,get<2>(x));
	}
