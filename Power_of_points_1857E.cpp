//
// Created by bravefart69 on 10/11/24.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
    ...
    for(int i = n-1;i>=0;i--) {
        suffix.pb(arr_copy[i]+suffix.back());
    }
    ...
    fr(i,0,n) {
        ...
        // bug(ind,count_less_equal,count_greater,x);

        ll anss = n;

        anss += (ll)x*count_less_equal;
        // bug(anss);
        anss -= prefix[count_less_equal];
        // bug(anss);
        anss+= suffix[count_greater];
        // bug(anss);
        ll test = (ll)x*count_greater;
        anss -= test;
        // bug(anss,test);
