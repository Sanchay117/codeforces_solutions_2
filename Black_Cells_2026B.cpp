//
// Created by bravefart69 on 5/11/24.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
    ...
    if(n%2==0) {
        ...
        fr(i,0,n) {
            ll chosen = arr[i];
            ll prev=-1 ,next=-1;
            for(ll i = chosen-1;i>-1;i--) {
                if(hashTable[i]==0) {
                    prev = i;
                    break;
                }
            }
            for(ll i = chosen+1;i<=10e18+1;i++) {
                if(hashTable[i]==0) {
                    next = i;
                    break;
                }
            ...
            ll dist_before = LLONG_MAX,dist_ahead = LLONG_MAX;
            if(prev!=-1) dist_before = chosen-prev;
            if(next!=-1) dist_ahead = next-chosen;
            ...
            while(j<n) {
                ...
                ll diff = arr[j+1]-arr[j];
                mn = max(mn,diff);
                j+=2;
            }

            if(dist_before!=LLONG_MAX) mn = max(dist_before,mn);
            if(dist_ahead!=LLONG_MAX) mn = max(dist_ahead,mn);
