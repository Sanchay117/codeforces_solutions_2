//
// Created by bravefart69 on 7/11/24.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
    ...
    while(l<=r) {
        if(l==r) {
            // bug(counter,arr[l],ans);
            if(arr[l]%2==counter%2) {
                ans+= ((arr[l]+counter)/2 - counter) + 1;
            }else {
                ans+= ((arr[l]+counter)/2 - counter) + 2;
                if(arr[l]==1 && counter == 0) ans--;
            }
            break;
        ...
        counter+=arr[l];
        // bug(l,r,counter,arr[l],arr[r],ans);
