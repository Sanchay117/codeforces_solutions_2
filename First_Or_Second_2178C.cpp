...
void solve(int tt) {
    ...
    for (auto x:arr) prefix.pb(x+prefix.back());
    int i = 0;
    for (auto x:arr) {
        if (i==0) prefix_abs.pb(x);
        else prefix_abs.pb(abs(x) + prefix_abs.back());
        i++;
    }
    ...
    for (int i=0;i<n;i++) {
        lli x = 0;
        if (i==0) {
            x = -(prefix.back()-prefix[1]);
            ans=max(ans,x);
        }else {
            x += prefix_abs[i] - (prefix.back()-prefix[i+1]);
            ans=max(ans,x);
        }
    }
