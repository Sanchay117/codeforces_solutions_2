...
void solve(int tt) {
    ...
    int n;cin >> n;
    vlli dangers(n);ArrInput(dangers,n);
    ...
    vector<plli> dp (n+1);
    dp[root] = {dangers[root - 1],dangers[root - 1]};
    ...
    while (!q.empty()) {
        ...
        for (int child:adj[parent]) {
            if (!visited[child]) {
                q.push(child);
                dp[child].F = max(dangers[child-1],dangers[child-1] - dp[parent].S);
                dp[child].S = min(dangers[child-1],dangers[child-1] - dp[parent].F);
                visited[child] = true;
            }
        }
    }
