...
bool possible(int m, int k, const vlli &b, const vlli &groups, const vlli &b_prefix) {
    ...
    for (int p = 0; p < k; ++p) {
        ...
        lli t = prefix_count;
        int pos = (int)(upper_bound(b.begin(), b.end(), t) - b.begin());

        lli sum_small = b_prefix[pos];
        lli rightsum = sum_small + (lli)(m - pos) * t;

        if (left_sum > rightsum) return false;
    }
