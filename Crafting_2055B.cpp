//
// Created by bravefart69 on 12/1/25.
//
#include <bits/stdc++.h>
#include <climits>
#include <fstream>
...
void solve(int tt) {
	...
	bool poss = true;
	vector<pii> list; // {ai,bi}

	fr(i,0,n) {
		list.emplace_back(arr[i],brr[i]);
	}

	int min = INT_MAX,minI = -1;
	int min2 = INT_MAX,minI2 = -1;
	fr(i,0,n) {
		if(arr[i]<min) {
			min = arr[i];
			minI = i;
	...
	fr(i,0,n) {
		if(arr[i]>= min && arr[i]<min2 && i!=minI) {
			min2 = arr[i];
			minI2 = i;
		}
	}

	int sub = 0;int greater = 0;

	fr(i,0,n) {
		if(arr[i]>brr[i])greater++;
	}
